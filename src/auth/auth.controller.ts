...
export const loginUser = async (req: Request, res: Response) => {
    try {
        ...
        res.status(200).json({ token, userId: user.id, email: user.email, userType: user.userType });
    } catch (error:any) {
...
export const passwordReset = async (req: Request, res: Response) => {
    try {
        ...
        const results = await sendNotificationEmail(email, "Password Reset", user.name, `Click the link to reset your password: <a href="http://localhost:5000/api/auth/reset/${resetToken}">Reset Password</a>`);
...
export const updatePassword = async (req: Request, res: Response) => {
    try {
        ...
        // Send reset email (you can implement this function)
        const results = await sendNotificationEmail(email, "Password Reset", user.name, "PassWord Reset Successfully");
